...
%     end
    ...
    for i = 1:size(cmatx, 1)
        ...
        df = [1 32]; % wani: study specific [1 n-1]
        
        if isfield(clusters, 'shorttitle'), fprintf(fid, '%s\t', clusters(i).shorttitle);end
        fprintf(fid, '%3.0f\t%3.0f\t%3.0f\t%3.0f\t%3.2f\t%3.0f\t%3.0f\t%3.2f\t%3.2f\t%3.2f\t', i, x(i), y(i), z(i), cmatx(i, 1), cmatx(i, 2), cmatx(i, 2).*prod(clusters(i).voxSize), spm_u(10^-abs(cmatx(i, 3)), df, 'T'), spm_u(10^-abs(cmatx(i, 3)), df, 'Z'), cmatx(i, 3)); % wani
