function clusters = cluster_table_aal3(clusters, varargin)
% function cluster_table_aal3(clusters, [opt] subclusters)
...
% 
% To use this, you need AAL atlas data 
% (you can download from the website 
%                           - https://www.gin.cnrs.fr/en/tools/aal/) 
%
% and canlab atlas data. 
%
% This is modified version from the functions "cluster_table_wani.m" and
% "cluster_table.m". (Belows are not modified)
%
% By Suhwan Gim (Oct, 2023)
%
% If you are using these lables in your paper, you need to cite
%
% : Rolls, E.T., Huang,	C.C., Lin, C.-P., Feng, J.,	Joliot,	M.,
%           2020.	Automated anatomical labelling atlas 3,	Neuroimage
%           206:116189. https://doi.org/10.1016/j.neuroimage.2019.116189
%
...
%     end
    ...
    % ========= aal3 setting
    atlas = which('ROI_MNI_V7_1mm.nii');%fullfile(atlas_basedir, 'ROI_MNI_V4.nii');
    MNID = spm_vol(atlas);
    load(which('ROI_MNI_V7_1mm_Border.mat')); %load(fullfile(atlas_basedir, 'ROI_MNI_V4_Border.mat'));
    load(which('ROI_MNI_V7_1mm_List.mat')); %load(fullfile(atlas_basedir, 'ROI_MNI_V4_List.mat'));   
    ...
    for i = 1:size(cmatx, 1)
        ...
        fprintf(fid, '%3.0f\t%3.0f\t%3.0f\t%3.0f\t%3.2f\t%3.0f\t%3.0f\t%3.6f\t', i, x(i), y(i), z(i), cmatx(i, 1), cmatx(i, 2), cmatx(i, 2).*prod(clusters(i).voxSize), cmatx(i, 3));
...
% ========= Suhwan's additional setting (AAL3) 

atlas = which('ROI_MNI_V7.nii');
MNID = spm_vol(atlas);
load(which('ROI_MNI_V7_Border.mat')); 
load(which('ROI_MNI_V7_List.mat')); 
MNIY = spm_read_vols(MNID);
fact = 1;
