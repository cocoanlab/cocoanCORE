% This script generates 88-region subcortical atlas, combining
% 1) 54-region scale-IV subcortical atlas from Tian et al., 2020, Nat Neuro,
% 2) 32-region multi-dataset cerebellum atlas from Nettekoven et al., 2023, Biorxiv,
...
atlas_cerebellum_orig = fullfile(gitdir, 'cerebellar_atlases/Nettekoven_2023/atl-NettekovenSym32_space-MNI152NLin6AsymC_dseg.nii');
tmpdir = tempname(tempdir); mkdir(tmpdir);
atlas_cerebellum = fullfile(tmpdir, 'atl-NettekovenSym32_space-MNI152NLin6AsymC_dseg.nii');
...
atlas_combined_vol.fname = fullfile(gitdir, 'cocoanlab/cocoanCORE/Canonical_brains/subcortex_comb/subcortex_comb_r88.nii');
spm_write_vol(atlas_combined_vol, atlas_combined);

rmpath(genpath(spmdir));

subcortex_comb_r88 = struct;
subcortex_comb_r88.dat(:,1) = 1:max(atlas_combined,[],1:3);
subcortex_comb_r88.descrip = {'Column 1: Original mask value, Column 2: Gross-anatomical labels, Column 3: Laterality', ...
...
cb_names = split(importdata(fullfile(gitdir, 'cerebellar_atlases/Nettekoven_2023/atl-NettekovenAsym32.lut')));
cb_names = cellfun(@(a) sprintf('%s-%sh', a(1:2), lower(a(3))), cb_names(:,end), 'un', false);
cb_names = strcat('Cb_', cb_names);
subcortex_comb_r88.names = [sctx_names; cb_names; {'PAG'; 'Brainstem'}];
subcortex_comb_r88.dat(:,2) = sum([contains(subcortex_comb_r88.names, 'THA'), ...
    contains(subcortex_comb_r88.names, {'HIP', 'AMY'}), ...
    contains(subcortex_comb_r88.names, {'CAU', 'PUT', 'NAc', 'GP'}), ...
    contains(subcortex_comb_r88.names, {'Cb'}), ...
    contains(subcortex_comb_r88.names, {'PAG'; 'Brainstem'})] .* (1:5), 2);
subcortex_comb_r88.dat(:,3) = sum([contains(subcortex_comb_r88.names, {'-lh'}) contains(subcortex_comb_r88.names, {'-rh'})] .* [-1 1], 2);
save(fullfile(gitdir, 'cocoanlab/cocoanCORE/Canonical_brains/subcortex_comb/subcortex_comb_r88_labels.mat'), 'subcortex_comb_r88');
