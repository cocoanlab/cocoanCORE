function [out, o2] = brain_activations_display_jj(r, varargin)
    ...
    if ischar(varargin{i})
        switch varargin{i}
            ...
            case {'poscm'}
                poscm = varargin{i+1};
                if size(poscm,1) == 2
                    poscm = interp1(0:1, poscm, 0:0.01:1);
                end
            case {'negcm'}
                negcm = varargin{i+1};
                if size(negcm,1) == 2
                    negcm = interp1(0:1, negcm, 0:0.01:1);
                end
            case {'colorbar'}
                do_colorbar = true;
...
% disply overlay
if do_montage
    if do_all && ~any(strcmp('cmaprange', varargin))
        varargin{end+1} = 'cmaprange';
        if isfield(out.colorbar, 'pos') && isfield(out.colorbar, 'neg')
            varargin{end+1} = [out.colorbar.pos([1 end],1).' out.colorbar.neg([1 end],1).'];
        elseif isfield(out.colorbar, 'pos') && ~isfield(out.colorbar, 'neg')
            varargin{end+1} = [out.colorbar.pos([1 end],1).'];
        elseif ~isfield(out.colorbar, 'pos') && isfield(out.colorbar, 'neg')
            varargin{end+1} = [out.colorbar.neg([1 end],1).'];
        end
    end
...
if do_colorbar
    ...
    if do_surface && ~do_all 
        ...
        q = 0.55; % scaling factor of height: figure size of do_all is 3/2 of do_surface.
    elseif do_all
        p = 0.05; % unit for determining position and size of colorbar
        q = 0.8; % scaling factor of height: figure size of do_all is 3/2 of do_surface.
    end
    
    sf = 1 / (1 + numel(fieldnames(out.colorbar)) * p); % scaling factor of overall size;
    cb_ax_pos = repmat({[sf, (1-q*sf)/2, (p*2/5)*sf, q*sf]}, 1, 2);
    cb_ax_pos{2}(1) = cb_ax_pos{2}(1) + p*sf;
    cb_pos = repmat({[sf, (1-q*0.6*sf)/2, (p*1/5)*sf, q*0.6*sf]}, 1, 2);
    cb_pos{2}(1) = cb_pos{2}(1) + p*sf;
    
    if isfield(out.colorbar, 'pos') && isfield(out.colorbar, 'neg')
        cb_ax_pos = cb_ax_pos(1:2);
        cb_pos = cb_pos(1:2);
        cb_lim = {out.colorbar.pos([1 end],1), out.colorbar.neg([1 end],1)};
        cb_map = {out.colorbar.pos(:,2:4), out.colorbar.neg(:,2:4)};
    elseif isfield(out.colorbar, 'pos') && ~isfield(out.colorbar, 'neg')
        cb_ax_pos = cb_ax_pos(1);
        cb_pos = cb_pos(1);
...
global poscm negcm color do_color do_region_color
    ...
    if ischar(vars{i})
        switch vars{i}
            ...
            case {'cmaprange'}
                do_cmaprange = true;
                cmaprange = vars{i+1};
                if numel(cmaprange) == 2; cmaprange = [cmaprange 0 0]; end
                cmaprange = sort(cmaprange, 'descend');
                cmaprange = cmaprange([4 3 2 1]);
        end
    else
...
if ~do_color
    if ~do_region_color
        if ~do_pruned && ~do_cmaprange
            o2 = addblobs_jj(o2, r, 'colorscheme', poscm, negcm); % A&B
        elseif do_pruned
            o2 = addblobs_jj(o2, r, 'colorscheme', poscm, negcm, 'cmaprange', [-2.8 -1.2 1.2 2.8]);
        elseif do_cmaprange
            o2 = addblobs_jj(o2, r, 'colorscheme', poscm, negcm, 'cmaprange', cmaprange);
        end
    elseif do_region_color
        for ii = 1:numel(r)
            o2 = addblobs_jj(o2, r(ii), 'color', color(ii,:)); 
        end
    end
else
    o2 = addblobs_jj(o2, r, 'color', color); % A&B
end

if dooutline, o2 = addblobs_jj(o2, r, 'outline', 'linewidth', 2, 'outline_color', [0 0 0]); end

end

function surface_light(gca, camlight_off)

out.h = get(gca, 'children');
set(out.h(2), 'BackFaceLighting', 'lit')
if ~camlight_off; camlight(-90,-20); end;
axis vis3d;

end

function out = draw_surface(r, out, hemisphere, vars)
...
switch hemisphere
    ...
    case 'left'
        ...
        if ~do_color
            switch surface_style
                case 'inflated'
                    [out.h_surf_L, out.colorbar] = cluster_surf_cocoan(r, 'underlay', which('surf_workbench_inflated_32k_Left.mat'), 'depth', depth, 'colormaps', poscm, negcm, 'prioritize_last', prioritize_last, 'refZ', cmaprange);
                case 'veryinflated'
                    [out.h_surf_L, out.colorbar] = cluster_surf_cocoan(r, 'underlay', 'fsavg_left', 'depth', depth, 'colormaps', poscm, negcm, 'prioritize_last', prioritize_last, 'refZ', cmaprange);
                case 'veryinflated_wb'
                    [out.h_surf_L, out.colorbar] = cluster_surf_cocoan(r, 'underlay', which('surf_workbench_very_inflated_32k_Left.mat'), 'depth', depth, 'colormaps', poscm, negcm, 'prioritize_last', prioritize_last, 'refZ', cmaprange);
            end
        else
            switch surface_style
                case 'inflated'
    ...
    case 'right'
        ...
        if ~do_color
            switch surface_style
                case 'inflated'
                    [out.h_surf_R, out.colorbar] = cluster_surf_cocoan(r, 'underlay', which('surf_workbench_inflated_32k_Right.mat'), 'depth', depth, 'colormaps', poscm, negcm, 'prioritize_last', prioritize_last, 'refZ', cmaprange);
                case 'veryinflated'
                    [out.h_surf_R, out.colorbar] = cluster_surf_cocoan(r, 'underlay', 'fsavg_right', 'depth', depth, 'colormaps', poscm, negcm, 'prioritize_last', prioritize_last, 'refZ', cmaprange);
                case 'veryinflated_wb'
                    [out.h_surf_R, out.colorbar] = cluster_surf_cocoan(r, 'underlay', which('surf_workbench_very_inflated_32k_Right.mat'), 'depth', depth, 'colormaps', poscm, negcm, 'prioritize_last', prioritize_last, 'refZ', cmaprange);
            end
        else
            switch surface_style
                case 'inflated'
