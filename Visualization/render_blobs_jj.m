function [blobhan, cmaprange, mincolor, maxcolor] = render_blobs_jj(currentmap, mymontage, SPACE, varargin)
...
for i = 1:length(varargin)
    if ischar(varargin{i})
        switch varargin{i}
            ...
            case 'colorscheme', docolorscheme = 1; poscm = varargin{i + 1}; negcm = varargin{i + 2};
    ...
    if wh_slice(j) ~= 0 % Wani added this if and end 8/11/12
        ...
        if any(slicedat(:))
            ...
            if docontour
                ...
                elseif ~dosplitcolor && ~docolorscheme
                    % color-mapped
                    Zscaled = Z;
                ...
                elseif docolorscheme
                    
                    Z_scaled = Z(:);
                    Z_scaled(Z(:) > 0) = (Z_scaled(Z(:) > 0) - cmaprange(3)) ./ (cmaprange(4) - cmaprange(3));
                    Z_scaled(Z(:) < 0) = (-Z_scaled(Z(:) < 0) - -cmaprange(2)) ./ (-cmaprange(1) - -cmaprange(2));
                    Z_scaled(Z_scaled > 1) = 1;
                    Z_scaled(Z_scaled < 0) = 0;
                    
                    slicecdat = NaN(numel(Z_scaled), 3);
                    slicecdat(Z(:) > 0, :) = poscm(round(Z_scaled(Z(:) > 0) .* (size(poscm,1)-1)) + 1, :);
                    slicecdat(Z(:) < 0, :) = negcm(-round(Z_scaled(Z(:) < 0) .* (size(negcm,1)-1)) + size(negcm,1), :);
                    slicecdat = reshape(slicecdat, [size(Z,1), size(Z,2), 3]);
